d3.csv("design5_2021.csv").then(function (data) {
    ...
    grade.append('rect')
        ...
        .on('mouseover', function (event, d) {
            d3.select(this)
                .style('stroke', '#c75d44') // Dark blue stroke
        ...
        .on('mouseout', function () {
            d3.select(this)
                .style('stroke', 'none'); // Revert to no stroke
    ...
    innerChart.append('line')
            .attr('x1', 0) // Starting x-coordinate at the leftmost point of the chart
            .attr('x2', 0) // Ending x-coordinate to keep it vertical
            .attr('y1', 0) // Starting y-coordinate at the top
            .attr('y2', height) // Ending y-coordinate at the bottom
            .attr('stroke', 'black') // Line color
            .attr('stroke-width', 1.5); // Line thickness for visibility
...
.append('rect') // Add a rectangle for Male
.attr('x', -10) // Position it to the left of the text
.attr('y', +2)
.attr('width', 15)
.attr('height', 15)
.attr('fill', '#68a0cf'); // Male bar color

chart.append('text')
.attr('class', 'axis-label')
.attr('x', width + margin.left + margin.right - 550) // Center below the red bar for Male
...
.append('rect') // Add a rectangle for Female
.attr('x', -14) // Position it to the left of the text
.attr('y', +2)
.attr('width', 15)
.attr('height', 15)
.attr('fill', '#f78c6c'); // Female bar color

chart.append('text')
.attr('class', 'axis-label')
.attr('x', width + margin.left + margin.right - 220) // Center below the blue bar for Female
.attr('y', height + margin.top + margin.bottom - 40)
.attr('text-anchor', 'middle')
.style('font-family', 'Arial')
.style('font-size', '14px')
